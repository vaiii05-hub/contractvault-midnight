...
As a mitigation, the application now automatically retries transient `InternalError` failures (up to 2 retries with a short delay) before surfacing an error to the user, which has significantly reduced how often this is encountered in practice.
